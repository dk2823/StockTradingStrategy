...
def getUniqueSymbols(orders_df):
  ...
  #   unique[symbol] = 1
  # return unique.keys()
  return list(orders_df.columns.values)
...
def createSymbolOrderDF(symbol, prices, orders_df, commission, impact):
  symbolOrderData = orders_df
  # Buy shares multiply by -1, because cash has to be reduced when BUY order is executed
  # symbolOrderData.loc[ symbolOrderData[symbol] > 0.0] = -1 *  symbolOrderData.loc[ symbolOrderData[symbol] > 0.0]
  symbolOrderData = symbolOrderData.rename(columns = {symbol:symbol + ":Shares"})
  ...
  symbolOrderData.drop(symbol,axis = 1,inplace = True)
...
def executeOrders(ordersRecord, symbols):
  ...
  for symbol in symbols:
      ordersRecord["cashVal"] = ordersRecord["cashVal"]  - (ordersRecord[symbol] * ordersRecord[symbol + ":Shares"])
...
def calculatePortVal(ordersRecord, prices, symbols, start_val):
  ...
  for symbol in symbols:
      ordersRecord["portVal"] = ordersRecord["portVal"] + ordersRecord[symbol] * ordersRecord[symbol+":Shares"] - (ordersRecord[symbol+":Commission"] - ordersRecord[symbol+":Impact"])
